...
/**
 *  Kamp (med hjemme- og bortelagets navner og deres respektive scorede m�l).
 */
struct Kamp {
    string hjemmeNavn,
           borteNavn;
...
int main ()  {
    ...
    while (!gKampene.empty())  {            //  S� lenge PEKERE i vectoren:
...
*/
    ...
    cout << "\nAntall i vectoren n�: " << gKampene.size() << "\n\n";
 ...
 *
 *  @param   kamp  - Structen som f�r fylt alle sine data (referanseoverf�rt)
 */
void kampLesData(Kamp & kamp)  {
    cout << "Hjemmelag:  ";       getline(cin, kamp.hjemmeNavn);
    cout << "Bortelag:   ";       getline(cin, kamp.borteNavn);
    kamp.hjemmeMaal = lesInt("Hjemmelagets antall m�l", 0, 20);
    kamp.borteMaal  = lesInt("Bortelagets antall m�l ", 0, 20);
}


/**
 *  Skriver ut p� skjermen ALLE en struct sine datamedlemmer.
 *
